...
export default class ItemsController {
  async index({ request, response }: HttpContext) {
    ...
    switch (sort) {
      ...
      case 'profit':
      default:
        // Sort by realistic max profit: margin * min(buy_limit, volume)
        // Using CASE for SQLite compatibility (no LEAST function)
        query.orderByRaw(
          `CASE WHEN item_prices.high_price IS NOT NULL AND item_prices.low_price IS NOT NULL AND items.buy_limit IS NOT NULL
           ...
           END ${orderDirection} ${nullsPosition}`
        )
        break
    }
    ...
    const data = items.map((item) => {
      ...
      const volume = item.$extras.volume ?? 0

      const profitMargin =
        highPrice != null && lowPrice != null ? highPrice - lowPrice : null

      // Use the lower of buy_limit or daily volume for realistic profit calculation
      // If volume is 0, max profit is 0 (can't trade items with no volume)
      let maxProfit: number | null = null
      if (profitMargin != null && buyLimit != null) {
        const effectiveLimit = Math.min(buyLimit, volume)
        maxProfit = profitMargin * effectiveLimit
      }
